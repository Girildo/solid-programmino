 ...
 */
function acceptVotes({ ballot, photos, format, preferences, issues }: AcceptArgs): {
  votes: Vote[]
  hadError: boolean
} {
  ...
  const hadError = issues.slice(before).some((issue) => issue.severity === "error")
 ...
 */
function reportVotesPerPhoto({ votes, author, at, format, issues }: CapArgs): void {
...
function pointsFor(format: ContestFormat, vote: Vote, preferences: number): number {
  return format.scoring.kind === "positional" ? preferences - vote.rank : format.scoring.points
}
...
function buildTables(
  ...
  for (const photo of photos.values()) {
    breakdowns.set(photo.id, Object.fromEntries(categories.map((c) => [c.key, 0])))
  }
  for (const { votes } of scored) {
    for (const vote of votes) {
      ...
      breakdown[vote.category] = (breakdown[vote.category] ?? 0) + pointsFor(format, vote, preferences)
    }
  }
  ...
  return format.tables.map((spec) => {
    ...
    for (const { votes } of scored) {
      for (const vote of votes) {
        ...
        points.set(vote.photoId, (points.get(vote.photoId) ?? 0) + pointsFor(format, vote, preferences))
      }
    }
...
export function tally(
  ...
  const { classified, issues: segmentIssues } = segment(comments, format, overrides)
  ...
  for (const ballot of parseBallots(classified, format)) {
    const { votes, hadError } = acceptVotes({ ballot, photos, format, preferences, issues })
    if (hadError) ballotsWithErrors += 1
    if (votes.length > 0) voters.add(ballot.comment.author.id)
    scored.push({ ballot, votes })
  }

  const tables = buildTables(photos, scored, format, preferences)
  ...
  if (zeroSeverity && headline && headline.rows.length > 0 && headline.rows[0]!.points === 0) {
    issues.push({
      severity: zeroSeverity,
      code: "everyPhotoAtZero",
      message: "Nessuna foto ha ricevuto punti: controlla il formato scelto e il numero di preferenze",
    })
  }

  const selfVoters = new Set(
    issues.filter((issue) => issue.code === "selfVote").map((issue) => issue.author?.id ?? ""),
  )
